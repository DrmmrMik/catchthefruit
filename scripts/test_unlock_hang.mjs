  ...
  ws.send(JSON.stringify({
    ...
    params: {
      expression: `(async () => {
        ...
        console.log("TEST: Calling preload.scene.start('MenuScene') directly...");
        try {
          preload.scene.start("MenuScene");
          console.log("TEST: scene.start returned synchronously!");
        } catch (e) {
          console.error("TEST: scene.start error:", e);
        }
      })()`,
      returnByValue: true
    }
...
ws.onmessage = async (event) => {
      ...
      params: {
        expression: `(() => {
          ...
          return {
            ...
            menuStatus: g.scene.getScene("MenuScene").scene.status
          };
        })()`,
        returnByValue: true
      }
  ...
  if (data.id === 4) {
    console.log('Scene status after direct start:', JSON.stringify(data.result.result.value, null, 2));
    chrome.kill();
    preview.kill();
    process.exit(0);
  }
