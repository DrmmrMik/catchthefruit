...
describe('Empirical Adversarial Verification: Milestone 4 Physics & Hitboxes (Challenger M4-1)', () => {
  ...
  describe('Suite 1: Fixed-Timestep Physics & Refresh-Rate Invariance', () => {
    ...
    it('stress tests delta scaling under extreme erratic jitter and frame drops', () => {
      ...
      // Generate erratic frames with jitter between 2ms and 50ms
      const deltas = [
  ...
  // ==========================================================================
  describe('Suite 2: Fall Duration Scaling Bounds Across Levels', () => {
    ...
    it('verifies GameScene initializes fallDurationMs directly without doubling multiplier', () => {
      ...
      // Verify that no 2x multiplier exists
  ...
  // ==========================================================================
  describe('Suite 3: Fruit Container HitArea Geometry & Touch Targets', () => {
    ...
    it('verifies centered geometry [-hitWidth/2, -hitHeight/2, hitWidth, hitHeight] contains (0,0) and boundary points (-24, -24)', () => {
      ...
      for (const word of testWords) {
        ...
        // Full 48px square check: all perimeter test points in 4px increments
  ...
  // ==========================================================================
  describe('Suite 4: Basket Bounds Clamping [0, 480]', () => {
  ...
  // ==========================================================================
  describe('Suite 5: Basket & Fruit Collision Detection Geometry', () => {
    it('accurately detects collision when fruit enters basket bounding rectangle', () => {
      const basketRect = new Phaser.Geom.Rectangle(240 - 48, 755 - 28, 96, 56);
  ...
  // ==========================================================================
  // 6. STANDALONE ORACLE HARNESS EXECUTION (.agents/challenger_m4_1/verify_m4.mjs)
  // ==========================================================================
  describe('Suite 6: Standalone Verification Oracle Execution', () => {
    it('executes verify_m4.mjs standalone script and verifies 0 errors in verification.log', async () => {
      const { execSync } = await import('child_process');
      const fs = await import('fs');
      const path = await import('path');

      const scriptPath = path.resolve('.agents/challenger_m4_1/verify_m4.mjs');
      const logPath = path.resolve('.agents/challenger_m4_1/verification.log');
      ...
      expect(logContent).toContain('Checks Failed         : 0');
      expect(logContent).toContain('Verdict               : APPROVE');
    });
  });
});
