#!/usr/bin/env python3
"""
generate_pwa_assets.py - High-fidelity icon & screenshot generator for Catch the Fruit PWA.
...
def create_gradient_canvas(width, height, top_color, bottom_color):
    """Creates a vertical linear gradient image (RGB)."""
...
def draw_apple_and_basket(draw, size):
    ...
    # Basket body
    bx0, by0 = cx - 140 * scale, cy + 80 * scale
    bx1, by1 = cx + 140 * scale, cy + 185 * scale
    # Basket trapezoid polygon
    basket_poly = [
    ...
    draw.ellipse(
        ...
        fill=(220, 38, 38, 255) # Slightly darker crimson red
    )
...
def generate_maskable_icon(target_size):
    ...
    # Vibrant orchard daylight gradient (Sky blue to emerald orchard green)
    # top: #0ea5e9 (14, 165, 233), bottom: #15803d (21, 128, 61)
...
def generate_any_icon(target_size):
    """
    Generates an 'any' icon with high visual pop.
    Full-bleed with subtle rounded badge or full frame suitable for any platform.
    """
    render_size = 1024
...
def generate_mobile_screenshot(output_path):
    ...
    Depicts the actual Phaser 4 arcade game screen:
      - Sky gradient background + rolling green hills
      - Top HUD: Topic A Phonics banner, prompt 'Catch words with ea that say /ē/!', score, stars
      - Falling fruit items with words: 'beach' (apple), 'bread' (banana), 'teach' (grapes)
      - Wicker basket catcher at bottom
      - Catch sparkle chime effect
    """
    width, height = 480, 800
    # 1. Sky gradient (#38bdf8 to #bae6fd)
    img = create_gradient_canvas(width, height, (56, 189, 248), (186, 230, 253))
    draw = ImageDraw.Draw(img)

    # 2. Fluffy clouds in sky
    ...
    # 3. Rolling green orchard hills at bottom
    # Back hill
    ...
    # 4. Top HUD Container
    hud_bg = (15, 23, 42, 210) # Dark translucent slate
    draw.rounded_rectangle([15, 20, 465, 125], radius=16, fill=hud_bg)

    # Try loading default font, or draw clean geometric text
    # Top info bar
    # Category badge
    ...
    # 5. Falling Fruits with Words
    # Fruit 1: Red Apple with target word "beach" (Correct catch target)
    ax, ay = 110, 260
    # Glow around correct fruit
    ...
    draw.line([(ax, ay - 35), (ax + 5, ay - 48)], fill=(120, 53, 15, 255), width=4) # Stem
    draw.polygon([(ax + 5, ay - 48), (ax + 24, ay - 55), (ax + 16, ay - 42)], fill=(34, 197, 94, 255)) # Leaf
    # Word label card below fruit
    ...
    # Fruit 2: Distractor Banana with word "bread" (short /ĕ/ sound!)
    bx, by = 250, 380
    # Banana curve
    ...
    # 6. Wicker Basket Catcher at Bottom
    kcx, kcy = 240, 690
    basket_poly = [
        (kcx - 100, kcy - 10),
        (kcx + 100, kcy - 10),
    ...
    # 7. Caught Fruit & Particle Burst
    # Peach falling into basket
    px, py = kcx, kcy - 30
    draw.ellipse([px - 28, py - 26, px + 28, py + 26], fill=(251, 146, 60, 255)) # Orange peach
    # Sparkles!
...
def main(output_root="public"):
    ...
    # 1. Generate Maskable Icons (192 and 512)
    ...
    # 2. Generate Any Icons (192 and 512)
    ...
    # 3. Generate Mobile Screenshot (480x800)
